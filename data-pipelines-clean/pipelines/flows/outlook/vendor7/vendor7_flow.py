...
sql_template = os.path.join(os.path.dirname(os.path.realpath(__file__)), "vendor7.sql")
copy_sql = render_copy_sql(sql_template_file=sql_template)
snowflake_table = "VENDOR7_REVENUE"
title_col = "ASSET_TITLE"


def Vendor7OutlookFlow(
    days_back=None,
    ENVIRONMENT="dev",
    **kwargs,
):
    partner_name = "vendor7"
    flow_name = "revenue"
    subject_line = "vendor7"
    attachment_title_filters = [r".*CLIENTNAME.*\d{4}\.xlsx"]
    with Flow(
        f"{partner_name.title()} Flow - {flow_name.upper()}",
        ENVIRONMENT=ENVIRONMENT,
        **kwargs,
    ) as flow:
        days_back = days_back or Parameter("days_back", default=30)
...
if __name__ == "__main__":
    ...
    print("Running Vendor7 TV Pipeline")
    flow = Vendor7OutlookFlow(days_back=vargs.days_back, ENVIRONMENT="dev")
