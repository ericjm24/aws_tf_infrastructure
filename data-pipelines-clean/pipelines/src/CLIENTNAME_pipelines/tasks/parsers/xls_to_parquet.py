...
class XlsToParquet(CLIENTNAMELambdaInvoke):
    def __init__(self, **kwargs):
        super().__init__(
            function_config_name="s3.pipelines_tf_state.xls_to_parquet_lambda_name",
            **kwargs
        )
    ...
    def set_payload(
        ...
        **kwargs
        ...
        if not target_key:
            if source_key.endswith((".xls", ".xlsx")):
        ...
        payload = {
            ...
            "xls_kwargs": b64encode(dumps(xls_kwargs).encode("utf-8")).decode("utf-8"),
        }
        if schema:
